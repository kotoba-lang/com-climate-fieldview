(ns climate-fieldview.main-test
  "Contract + behavioral test for the climate_fieldview-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "climate_fieldview-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'climate-fieldview.main-test)]
