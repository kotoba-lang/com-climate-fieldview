(ns climate-fieldview.main
  "Kotodama WASM entrypoint for the Climate FieldView clean-room actor (L4) — Clojure port.
  ...
  facts (`climate_fieldview.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "climate_fieldview")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"climate_fieldview.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "climate_fieldview-compat" :tier tier :entities entities} 200])
...
(defn start! [] :climate_fieldview-compat/ready)
